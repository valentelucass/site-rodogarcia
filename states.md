...
- O pre-flight de produção valida `FFMPEG_PATH` e `FFPROBE_PATH` antes de instalações ou builds. Ambos devem apontar para executáveis absolutos, existentes e fora do repositório; assim uma configuração de metadados de mídia incompleta não chega ao hardening ou ao runtime.
...
- Após isolar a configuração do teste de manifesto antes do runtime do CMS, `cms/backend` passou em `mvnw.cmd -B -ntp clean verify` com `NODE_ENV=production` e 188 testes; `node scripts/tests/test-production-operations.js` também passou. O pre-flight agora interrompe imediatamente quando `FFPROBE_PATH` estiver ausente.
- Após configurar localmente um `FFPROBE_PATH` absoluto e validar o binário, `scripts/validate-production-inputs.ps1` passou com o ambiente de produção carregado; nenhum serviço foi iniciado ou reiniciado.
