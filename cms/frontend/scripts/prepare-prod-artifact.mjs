...
const nextDir = path.join(cmsRoot, nextBuildDirectoryName);
...
await requireDirectory(standaloneDir, "Build standalone do Next do CMS");
await requireDirectory(standaloneAppDir, "Raiz standalone do CMS");
await requireDirectory(staticDir, "Assets estaticos do Next do CMS");

const buildId = (await readFile(path.join(nextDir, "BUILD_ID"), "utf8")).trim();

await rm(outputDir, { recursive: true, force: true });
// Com `externalDir`, o Next preserva o caminho `cms/frontend` dentro de
...
try {
  ...
  // O diretório public é opcional para um app Next standalone.
}
...
console.log(`Artefato produtivo do CMS atualizado: ${path.relative(cmsRoot, outputDir)}`);
